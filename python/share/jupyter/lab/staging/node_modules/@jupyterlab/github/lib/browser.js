...
import { parsePath } from './contents';
/**
    ...
    constructor(browser, drive) {
        ...
        this._openGitHubButton = new ToolbarButton({
            onClick: () => {
                ...
                const localPath = this._browser.model.manager.services.contents.localPath(this._browser.model.path);
                const resource = parsePath(localPath);
                url = URLExt.join(url, resource.user);
                if (resource.repository) {
                    url = URLExt.join(url, resource.repository, 'tree', 'master', resource.path);
                }
                window.open(url);
            },
        ...
        this._launchBinderButton = new ToolbarButton({
            onClick: () => {
                ...
                const localPath = this._browser.model.manager.services.contents.localPath(this._browser.model.path);
                const resource = parsePath(localPath);
                const url = URLExt.join(MY_BINDER_BASE_URL, resource.user, resource.repository, 'master');
    ...
    _onUserChanged() {
        ...
        this._browser.model.cd(`/${this.userName.name}`).then(() => {
            ...
            if (document.activeElement === document.body) {
                const listing = this._browser.layout.widgets[2];
                listing.node.focus();
            }
        });
    }
    ...
    _onPathChanged() {
        ...
        if (resource.path === '') {
            const item = find(this._browser.model.items(), i => {
                return (i.name === 'requirements.txt' ||
