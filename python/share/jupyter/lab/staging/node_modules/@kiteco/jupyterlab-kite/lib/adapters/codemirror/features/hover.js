...
export class Hover extends CodeMirrorLSPFeature {
    constructor() {
        ...
        this.name = 'Hover';
        this.on_hover = async () => {
            const hover = await this.connection.getHoverTooltip(this.virtual_position, this.virtual_document.document_info, false);
            return hover;
        };
    ...
    editor_range_for_hover(range) {
        ...
        if (range) {
            let cm_editor = this.virtual_editor.get_editor_at_root_position(character);
            return this.range_to_editor_range(range, cm_editor);
        }
        else {
            // construct range manually using the token information
            let cm_editor = this.virtual_document.root.get_editor_at_source_line(character);
